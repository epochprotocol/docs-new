...
**Audience:** Integrators, partners, and investors only.
**Not included:** Internal engineering runbooks, service deep dives, deployment topology, repo maps, env vars, or ops content.

**Security principle:** Document *what integrators need to build*, not *how Epoch runs internally*. When in doubt, omit.

> Status: Validated against source code on 2026-06-08. See [Validation Notes](#validation-notes) for what was verified and corrected.
...
- Supported chains (mainnet + testnet) — names and chain IDs only (see [Validation Notes](#validation-notes) for verified list)
...
**Source:** Public SIO API only ([epoch-sio/docs/openapi.yaml](../epoch-sio/docs/openapi.yaml), [epoch-sio/README.md](../epoch-sio/README.md) — sanitized).
